...
MUST_Fungi_Vidiano_2019

View(data.frame(sample_data(MUST_Fungi_Vidiano_2019)))

MUST_Fungi_Vidiano_2019

adiv <- plot_richness(MUST_Fungi_Vidiano_2019, measures=c("Shannon"))

adiv <- plot_richness(MUST_Fungi_Vidiano_2019, x="stage2", measures=c("InvSimpson", "Shannon"), color="vinification", shape ="vinification")
...
good <- MetaCommunity(t(MUST_Fungi_Vidiano_2019@otu_table))$SampleCoverage.communities
...
alpha_wide_fact <- merge(alpha_wide, data.frame(sample_data(MUST_Fungi_Vidiano_2019)), by = "row.names")
...
##FUNGI
MUST_Fungi_Vidiano_2019.NMS <- MUST_Fungi_Vidiano_2019
sample_names(MUST_Fungi_Vidiano_2019.NMS) <- paste("samp",sample_names(MUST_Fungi_Vidiano_2019), sep = "")

myrichness <- estimate_richness(MUST_Fungi_Vidiano_2019.NMS, split = TRUE, measures = c("Shannon", "InvSimpson", "Observed")) # calculate the Inverse 

mysamdat <- data.frame(sample_data(MUST_Fungi_Vidiano_2019.NMS)[,c("vinification","stage","stage2")]) # obtain the sample info data.frame
