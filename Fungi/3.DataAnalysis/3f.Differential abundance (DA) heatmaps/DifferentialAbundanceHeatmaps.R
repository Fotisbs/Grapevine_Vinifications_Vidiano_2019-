...
MUST_Fungi_Species_2 <- tax_glom(MUST_Fungi_Vidiano_2019, taxrank = "Species")

MUST_Fungi_Species_2

psdt <- MUST_Fungi_Species_2

##transform phyloseq object raw counts to relative abundance##
psdt_ra <- transform_sample_counts(psdt, function(x) x / sum(x))

##Define the number of OTUs to be used in the analysis, 88 ASVs in our case   
mynumOTUs <- 88
...
write.table(mat_fr_plot_agg, file = "Heatmap_Fungi_Vidiano_Vinifications.txt")
...
cairo_pdf(paste("Heatmap_Fungi_Vidiano_Vinifications.pdf", sep = ""), width =17, height =35)
...
##you can write the table##
write.table(mat_fr_plot_agg, file = "Heatmap_Fungi_Vidiano_Stages.txt")
...
cairo_pdf(paste("Heatmap_Fungi_Vidiano_Stages.pdf", sep = ""), width =17, height =35)
