############################################################
# Figure 5b: Stage-specific transcript heatmap — Middle (S2)
#
# Comparison:
#   Inoculated versus Spontaneous fermentations at S2
#
...
#
# Input files:
#   Final_names_middle.txt
#   TheDesign2ViniMiddle.txt
#
# Output:
#   Figure5b_S2_GeneExpressionHeatmap.pdf
############################################################
...
if (!all(colnames(cts) %in% rownames(Information))) {
  stop(
    "Sample names in Final_names_middle.txt and ",
    "TheDesign2ViniMiddle.txt do not match."
  )
}
...
dds_2VinMiddle_Func <- DESeqDataSetFromMatrix(
...
cnts_perc <- 100 * decostand(
  counts(dds_2VinMiddle_Func),
  MARGIN = 2,
  method = "total"
)

# Retain genes whose summed relative contribution across
# the four S2 samples is at least 0.1 percentage points
keep_genes <- rowSums(
  cnts_perc,
  na.rm = TRUE
) >= 0.1

dds_2VinMiddle_Func <- dds_2VinMiddle_Func[
  keep_genes,
]
...
message(
  ...
  " genes retained from ",
  length(keep_genes),
  " annotated genes for the S2 analysis."
)
...
dds_2VinMiddle_Func <- DESeq(
  dds_2VinMiddle_Func
)

saveRDS(
  dds_2VinMiddle_Func,
  file = "dds_2VinMiddle_Func.rds"
)
...
write.table(
  deseq_results_df,
  file = "S2_DESeq2_results_all.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  significant_genes,
  file = "S2_DESeq2_results_significant.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
message(
  ...
  " genes met padj < 0.05 and |log2FC| > 1 at S2."
)
...
# Export the genes selected for Figure 5b
...
write.table(
  ...
  file = "S2_DESeq2_top20_by_adjusted_pvalue.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  normalized_counts,
  file = "S2_DESeq2_normalized_counts.txt",
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
...
write.table(
  ...
  file = "S2_Figure5b_top20_normalized_counts.txt",
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
...
write.table(
  heatmap_matrix,
  file = "S2_Figure5b_top20_range_scaled_values.txt",
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
...
pdf(
  file = "Figure5b_S2_GeneExpressionHeatmap.pdf",
  width = 10,
  height = 10
)
