...
##NMDS##
##ordination matrix
ord.nmds.bray1 <- ordinate(MUST_Bacteria_Vidiano_2019_Merged_100, method="NMDS", distance="bray")

#Vinification
plot_ordination(MUST_Bacteria_Vidiano_2019_Merged_100, ord.nmds.bray1, color="vinification", shape ="", label = "", title=paste("NMDS (stress ",round(ord.nmds.bray1$stress, 2),")", sep = "")) + geom_point(size = 4) + scale_color_manual(values=c("#984EA3","#FF7F00","#33A02C"))

##Stage
plot_ordination(MUST_Bacteria_Vidiano_2019_Merged_100, ord.nmds.bray1, color="stage", shape ="", label = "", title=paste("NMDS (stress ",round(ord.nmds.bray1$stress, 2),")", sep = "")) + geom_point(size = 4) + scale_color_manual(values=c("#984EA3","#FF7F00","#33A02C","#80b1d3","Blue"))
...
mytax_tbl <- data.frame(tax_table(MUST_Bacteria_Vidiano_2019_Merged_100))
...
my_sel_var <- factor(MUST_Bacteria_Vidiano_2019_Merged_100@sam_data$vinification)

myterr_sel <- as.numeric(my_sel_var)

cairo_pdf("Bacteria_Vinification_NMDS.pdf", height = 6, width = 7)
...
my_sel_var <- factor(MUST_Bacteria_Vidiano_2019_Merged_100@sam_data$stage)

myterr_sel <- as.numeric(my_sel_var)

cairo_pdf("Bacteria_Stages_NMDS.pdf", height = 6, width = 7)
